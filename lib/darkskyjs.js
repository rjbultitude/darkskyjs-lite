...
( function( name, root, factory ) {
    ...
    */
    ...
    var methodError = 'method key must be a string';
    var JSONError = 'invalid JSON. Check the proxy is working correctly';
    ...
    DarkSky.prototype.isJsonString = function isJsonString(str, appCallback) {
      try {
          JSON.parse(str);
      } catch (e) {
            console.warn(e, JSONError);
            appCallback({
                error: true,
                type: 'JSON error',
                msg: JSONError
    ...
    DarkSky.prototype.processData = function processData(forecasts, locations, fnKey, appCallback) {
        ...
        for ( var i = 0, length = forecasts.length; i < length; i++ ) {
            if (this.isJsonString(forecasts[i], appCallback)) {
    ...
    DarkSky.prototype.checkData = function checkData( forecasts, locations, dataType, appCallback ) {
        if (!forecasts || !forecasts[0]) {
            throw new Error( darkSkyServiceError );
        }
    ...
    DarkSky.prototype.initAllReqs = function initAllReqs(locations, dataType, appCallback) {
        ...
        return Promise.all( validLocData ).then( function success( forecasts ) {
            try {
